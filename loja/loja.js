...
async function co3AplicarCupom(){
  ...
  try{
    const {data:cupom,error}=await sb.from('cupons').select('id,nome,desconto,usos_restantes,ativo').eq('nome',codigo).eq('ativo',true).single();
    ...
    if(usado?.length){
      co3CupomAtivo=null;
      msg.style.color='var(--red)';
      msg.textContent='Você já utilizou este cupom.';
      co3UpdateResumo();
      return;
    }
    co3CupomAtivo=cupom;
    ...
    msg.textContent='Cupom aplicado. O desconto final será validado ao finalizar.';
    co3UpdateResumo();
  }catch(e){
